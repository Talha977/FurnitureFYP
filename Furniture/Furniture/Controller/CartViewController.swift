...
class CartViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        cartListView.register(UINib(nibName: "FurnitureCartCell", bundle: nil), forCellReuseIdentifier: "FurnitureCartCell")
...
extension CartViewController :UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "FurnitureCartCell", for: indexPath) as! FurnitureCartCell
        
        cell.beerData = Global.sharedInstance.cart[indexPath.section]
        let item = CartItem()
        
        let imageIndex = Global.sharedInstance.cart[indexPath.section].imageIndex
        cell.itemIcon.image = FurnitureModel.shared.Images[imageIndex!]
...
extension CartViewController{
    ...
    @objc func bookbtn1(_ sender : UIButton){
        guard let cell = sender.superview?.superview?.superview as? FurnitureCartCell else {
            return
        }
